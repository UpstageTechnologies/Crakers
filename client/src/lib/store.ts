...
interface StoreType {
  ...
  resetFavorite: () => void;
  // // Compareproduct
  compareProduct: CartProduct[];
  addToCompare: (product: ProductProps) => Promise<void>;
...
export const store = create<StoreType>()(
  persist(
    (set) => ({
      ...
      addToCompare: (product: ProductProps) => {
    return new Promise<void>((resolve) => {
     set((state: StoreType) => {
      const isCompare = state.compareProduct.some(
        (item) => item._id === product._id
      );

      return {
        compareProduct: isCompare
          ? state.compareProduct.filter(
              (item) => item._id !== product._id
            )
          : [...state.compareProduct, { ...product }],
      };
    });
...
removeFromCompare: (productId: number) => {
  set((state: StoreType) => ({
    compareProduct: state.compareProduct.filter(
      (item) => item._id !== productId
    ),
  }));
},
